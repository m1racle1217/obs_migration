...
可以用配置或命令行开关启动本地 Web 控制台。控制台是无需前端构建工具的暗蓝科技风 Operations Shell，包含“任务仪表盘”“配置中心”“目录浏览”“日志 / 报告”四个区域。
...
- 登录成功后浏览器会在本机记住登录状态；点击“退出登录”会清除本地标记和服务端 session。
- Web 控制台启动后不会自动开始迁移任务；请在“任务仪表盘”中新增任务并点击“启动任务”，或调用 `POST /api/tasks/{task_id}/start`。
- 任务仪表盘支持进程内多任务并行管理，每个任务可单独启动、暂停、继续、停止，并可运行中调整上传/检查/扫描/分片并发。
- 目录浏览是文件管理器式窗口，支持后退、前进、上一级、刷新、单击选中、双击进入，并可将选中项加入迁移列表或填入任务配置。
- 兼容旧任务控制 API：`/api/task/status` 与 `/api/task/start|pause|resume|stop` 仍可使用。
