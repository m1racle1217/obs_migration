...
- 🗂️ **目录浏览更直观**：启动菜单可直接浏览源端 / 目标端，本地目录和对象存储桶 / 前缀都支持分页查看
...
启动后会进入配置菜单。一级菜单支持直接按热键操作，常用热键如下：
...
源端 / 目标端浏览会根据当前配置类型自动选择浏览方式：

- `type = s3`：列举桶、进入前缀目录、上一层、分页查看对象。
- `type = local`：浏览当前运行机器可访问的本地目录；Windows / Linux 路径都支持，但必须是运行机器能访问到的路径或已挂载路径。

浏览界面常用按键如下：

| 按键 | 作用 |
| --- | --- |
| 数字 | 进入对应目录；文件只展示属性，不会传输 |
| `S` | 保存当前位置到 `config.ini` |
| `B` | 返回上一层 |
| `N` | 下一页 |
| `P` | 上一页 |
| `R` | 刷新当前页和后台统计结果 |
...
`保存当前位置` 不会上传、下载或复制文件，只会把当前浏览位置写回配置：

- 浏览源端时保存，会更新 `[SOURCE]` 的 `path` 或 `bucket/prefix`。
- 浏览目标端时保存，会更新 `[TARGET]` 的 `path` 或 `bucket/prefix`。

浏览列表只展示当前目录的直接子项，不递归展开子目录。界面会显示 `当前显示: 起始-结束 / 对象数量: 总数`；对于对象很多的 S3 目录，程序会先显示当前页，总数在后台统计，完成后按 `R` 刷新即可看到准确总数。
