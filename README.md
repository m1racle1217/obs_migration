...
> 这里的 `s3` 指 **S3 协议兼容对象存储**。  
> 当前远端实现基于 **华为 OBS Python SDK**，OBS 为优先支持场景；其他 S3 兼容服务是否可用，取决于其对相关接口的兼容程度。
...
| 断点续传 | SQLite checkpoint + SDK 级分片续传 |
| 索引跳过 | 目标端索引缓存命中时快速跳过 |
| HEAD / ETAG 校验 | 支持更严格的存在性与一致性检查 |
| 实时仪表盘 | 展示扫描、上传、缓存命中、速率等指标 |
| 报告输出 | 生成 CSV 明细和 JSON 汇总 |
